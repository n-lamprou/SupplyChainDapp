...
contract SupplyChain is  FarmerRole, DistributorRole, RetailerRole, ConsumerRole{
  ...
  modifier onlyOwner() {
    require(msg.sender == contractOwner, "Contract owner action only");
    _;
  }

  // Define a modifier that verifies the Caller
  modifier verifyCaller (address _address) {
    require(msg.sender == _address, "Caller must own item");
    _;
  }
  ...
  modifier harvested(uint _upc) {
    require(items[_upc].itemState == State.Harvested, "Item state should be harvested");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Processed
  modifier processed(uint _upc) {
    require(items[_upc].itemState == State.Processed, 'Item state should be processed');
    _;
  }
  
  // Define a modifier that checks if an item.state of a upc is Packed
  modifier packed(uint _upc) {
    require(items[_upc].itemState == State.Packed, 'Item state should be packed');
    _;
  }

  // Define a modifier that checks if an item.state of a upc is ForSale
  modifier forSale(uint _upc) {
    require(items[_upc].itemState == State.ForSale, 'Item state should be forSale');
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Sold
  modifier sold(uint _upc) {
    require(items[_upc].itemState == State.Sold, 'Item state should be sold');
    _;
  }
